...
import { setCurrentUser, type UserRole, doctors } from "@/lib/store";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await api.login({ ...formData, role });
          ...
          <div className="flex gap-2 mb-6 bg-slate-100 p-1 rounded-lg">
            {(["patient", "doctor", "admin"] as UserRole[]).map((r) => (
              <button
                key={r}
                onClick={() => setRole(r)}
                className={`flex-1 py-1.5 px-2 rounded-md text-xs font-medium transition-all ${role === r
